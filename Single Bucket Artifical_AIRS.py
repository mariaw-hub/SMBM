...
data_art = pd.read_csv('Climate_scenario_airs.csv', sep=',')
...
#data_art['Year'] = data_art.index.year

PET = pd.read_csv('Infiltration_AIR070420.csv', sep='\t')
...
data_art = pd.concat([data_art, PET['PET']], axis=1)
data_art['P3'] = data_art['Rain'] *0.9
...
#%%
PET_art = pd.read_csv('Infiltration_AIR070420.csv', sep='\t') #PET with 2C Temperature change for CC3
...
#%%
art_ndvi_airs = pd.DataFrame()
 #ndvi values give values from January 
art_ndvi_airs['NetP1'] = data_art['P1'] - data_art['PET']
art_ndvi_airs['3M1'] = art_ndvi_airs.iloc[:,0].rolling(window=3).sum() #december = january ndvi

art_ndvi_airs['NetP2'] = data_art['P2'] - data_art['PET']
art_ndvi_airs['3M2'] = art_ndvi_airs.iloc[:,2].rolling(window=3).sum()

art_ndvi_airs['NetP3'] = data_art['P3'] - PET_art['PET']
art_ndvi_airs['3M3'] = art_ndvi_airs.iloc[:,4].rolling(window=3).sum()


art_ndvi_airs['ndvi1'] = 0.51617 + 0.00051*art_ndvi_airs['3M1'] #from NDVI & Net P regression 
art_ndvi_airs['ndvi2'] = 0.51617 + 0.00051*art_ndvi_airs['3M2']
art_ndvi_airs['ndvi3'] = 0.51617 + 0.00051*art_ndvi_airs['3M3']

cols = ['ndvi1','ndvi2', 'ndvi3']
art_ndvi_airs[art_ndvi_airs[cols] < 0] = 0        

art_ndvi_airs['Kc1']= 2.03*art_ndvi_airs['ndvi1'] - 0.15 #from NDVI & Net P regression
art_ndvi_airs['Kc2']= 2.03*art_ndvi_airs['ndvi2'] - 0.15 
art_ndvi_airs['Kc3']= 2.03*art_ndvi_airs['ndvi3'] - 0.15 

art_ndvi_airs = art_ndvi_airs.loc['2011-12-01':'2018-11-30'] #equals january to december
#%%
...
art_ndvi_airs = art_ndvi_airs.set_index(data_art.index)
PET_art = PET_art.loc['2012-01-01':'2018-12-31'] #FILL IN THAT GAP IN 2017-OCT
...
wp=0.04 #wilting point
Zr=888 # Rooting depth
Pc=0.29 #depletion fraction 
#%% calculate other parameters
TAW =  (fc - wp) * Zr #* (1-B) + (fc - 0.5*wp)*Ze *B  #total available water (mm)
RAW = Pc * TAW               #readily available water (mm)
#P = Rain
P=P2
PET = RET * art_ndvi_airs['Kc2']  
             
#PET = PET_art['PET'] * art_ndvi_airs['Kc3'] #for artifical PET scenario
#%%MODEL RUNS FROM 2012 - 2019 for the extreme climate scenarios 
...
#SM_mod = SM_mod.loc['2014-01-01':'2016-12-31'] 
SM_art_air2= SM_mod
...
#SMD_M_air = SMD_s.resample('M').mean()
#SMD_M_air = SMD_M_air.mean(axis=1)

#%% create a mean SMD and mean SM of all the MCs to plot over 2 years

t=SM_art_air1.index
diff_air1 = data_art.P2 - data_art.P1
diff_air2 = data_art.P1 - data_art.Rain
diff_air3= data_art.P3 -data_art.Rain

months = mdates.MonthLocator()
#%%
y=SM_mod_airs_M[29].loc['2012-01-01':'2018-12-31']
#%% Plot standard deviation of all 1000 Models around the best model plot as + - 1 STD

SM_mod_airs_M_std = SM_mod_airs_M.std(axis=1)
#SM_mod_airs_M_std=SM_mod_airs_M_std[SM_mod_airs_M_std.index.isin(best_model_c.index)] 
SM_mod_airs_M_std = SM_mod_airs_M_std.loc['2012':'2018']
...
ax1.fill_between(SM_mod_airs_M_std.index,y + SM_mod_airs_M_std, y - SM_mod_airs_M_std, color = 'grey', alpha = 0.6)
ax1.set_ylabel('VMC (m $\mathregular{^3}$/ m $\mathregular{^3}$)', fontsize = 'small', fontweight='bold')
ax1.tick_params(axis= 'both', which ='major', labelsize='8')
ax1.set_ylim(0,0.3)
ax1.axhline(0.07, color='black', alpha = 0.5)
...
ax3.plot(SM_art_air1[0], color = 'orange')
ax3.tick_params(axis= 'both', which ='major', labelsize='8')
ax3.set_ylim(0,0.3)
ax3.axhline(0.07, color='black', alpha = 0.5)
ax3.xaxis.set_minor_locator(months)
ax3.yaxis.set_ticks(np.arange(0,0.4,0.1))

ax4 = ax3.twinx()  # instantiate a second axes that shares the same x-axis
ax4.invert_yaxis()
ax4.bar(t, data_art.P1, color='black', width = 20, alpha = 0.6, label = 'Precipitation')
ax4.bar(t, diff_air2, color='red', bottom = data_art.Rain, width = 20,label = 'Precipitation')
...
ax7.plot(SM_art_air2[0], color = 'orange')
ax7.tick_params(axis= 'both', which ='major', labelsize='8')
ax7.axhline(0.07, color='black', alpha = 0.5)
ax7.set_ylim(0,0.3)
ax7.xaxis.set_minor_locator(months)
ax7.yaxis.set_ticks(np.arange(0,0.4,0.1))
...
ax8.bar(t, diff_air1, color='green', bottom = data_art.P1, width = 20,label = 'Precipitation')
ax8.bar(t, diff_air2, color='red', bottom = data_art.Rain, width = 20,label = 'Precipitation')
...
#ax10.bar(t, diff_air3, color='red', bottom = data_art.Rain, width = 20,label = 'Precipitation')
ax10.set_xlabel('Year', fontsize = 'small', fontweight='bold')
...
plt.show()
#%%%
best_model['Scenario'] = 'SH'
bm=pd.DataFrame(best_model.loc['2012':'2018'])

SM_art_air1['Scenario'] = 'A'
SM_art_air2['Scenario'] = 'B'
SM_art_air3['Scenario'] = 'C'

Art_C =pd.concat([bm, SM_art_air1,SM_art_air2,SM_art_air3])

plt.style.use('bmh')
ax1=sns.violinplot(x='Scenario', y=0, data=Art_C, linewidth = 0.6, hue='Scenario', dodge=False) 
ax1.axhline(0.07, color='black',alpha=0.4)


#%%

stats.ks_2samp(best_model[0].loc['2012':'2018'], SM_art_air3[0])
